...
function recount_form($items = array(),$auto = array(),$cat = array(),$next_trans,$user_x){
	$CI =& get_instance();
		$CI->make->sDivCol();
			$CI->make->sDivRow();
				//left search
				...
				$CI->make->sDivCol(12);
					$CI->make->sBox('primary',array('style'=>''));
						$CI->make->sBoxBody();				
							...
							$CI->make->H(5,' Manual count Product List',array('style'=>''));
							$CI->make->sDivRow();
								$CI->make->sDivCol();
											...
											$CI->make->sDivCol();
												...
												$CI->make->sDiv(array('class'=>'table-responsive','style'=>'height:250px;overflow:auto'));
													$CI->make->sTable(array('class'=>'table table-hover','id'=>'recount_list'));
														...
														//	$CI->make->th('Actual Count',array('style'=>'','align=center'));
															$CI->make->th('Currently Sold',array('style'=>'','align=center'));
												...
												$CI->make->sTableBody(array('id'=>'r_tblautoprod'));
													...
													//$role = "Security";
														...
														// }
														$recount_link = "";	
														if($v->recount == 1  && is_null($v->UserID)){
														...
														$CI->make->sRow();
															$CI->make->td($recount_link." <span id='warning_$no'></span>");
														...
														//	$CI->make->td($v->SellingArea);
															...
															$actual_sales = $CI->operation->getCurrentSales($v->ProductID);
															$CI->make->td($actual_sales);
															$CI->make->hidden('csold_'.$no,$actual_sales);
															$CI->make->hidden('userdb'.$no,$v->UserID);
															$CI->make->hidden('Barcode_cc_'.$no,$v->Barcode);
															$CI->make->td("<input type='hidden' id='ad_qty2_$no' disabled class='adjustment_qty' placeholder='Adjustment Quantity'>"."<input type='hidden' id='mc_$no'>");
															$CI->make->td("<input type='hidden' id='unit_cost2_$no' disabled value='".$v->CostOfSales."' class = 'allunitcost'>");
															$CI->make->td("<input type='hidden' id='total_$no' disabled value='' class = 'allunitcost'>");
...
function recount_reload($items = array(),$next_trans,$user_x){
$CI =& get_instance();
	$CI->make->sDivCol();
		$CI->make->sDivRow();
				$CI->make->sRow();
		...
		$CI->make->sDivRow();
			$CI->make->sForm("",array('id'=>'auto_form'));				
				$CI->make->sDivCol();
					...
					$CI->make->input('','txtcatprod','','Search Product',array('class'=>'','id'=>'txtautoprod'));
					$CI->make->sDiv(array('class'=>'table-responsive','style'=>'height:400px;overflow:auto'));
						$CI->make->checkbox('Disable Curretly Sold','chk_discursold');
					...
					$CI->make->sTableBody(array('id'=>'tblautoprod'));
						...
						$role = "Security";
							...
							$CI->make->sRow();
								...
								//$CI->make->td($v->SellingArea);
								$CI->make->td("
								<input type='hidden' id='damaged_$no' value='".$v->Damaged."'/>
								<input type='hidden' id='cos_$no' value='".$v->CostOfSales."'/>
								<input type='hidden' id='sysinv_$no' value='".$v->SellingArea."'/>
								<input type='text' class='actual-count' id='actual_$no'/>");
								$actual_sales = $CI->operation->getCurrentSales($v->ProductID);
								$CI->make->td($actual_sales);
								$CI->make->td("<input type='hidden' id='ccqty_$no' class='ccqty' value='' disabled='disabled'>"."<input type='hidden' id='mc_$no'>"."<input type='text' id='ad_qty2_$no' hidden value=''  class='adjustment_qty' placeholder='Adjustment Quantity'>");
								// $CI->make->td("<input type='text' id='ad_qty2_$no' hidden value=''  class='adjustment_qty' placeholder='Adjustment Quantity'>");
								$CI->make->td("<input type='hidden' id='unit_cost2_$no' disabled value='' class = 'allunitcost'>");
