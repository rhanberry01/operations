...
// where (n.SellingArea < 0 and v.consignor != 1) AND pos.uom = 'PC' ";
	...
	function r_add_prod_cc($prod = NULL){
		...
		if($connected){	
			...
			$sql2 = "Select * from TempRecount where Barcode = '$prod' AND Branch = '$user_branch'";
			$this->bdb->trans_begin();
			...
			{
				$sql = "Insert into TempRecount(ProductID,ProductCode,Barcode,Description,uom,Branch,GlobalID,UserID)
	...
	function r_get_cc(){
		...
		//	if($connected){
		//		$this->bdb->trans_start();		
				$sql = "select a.*,b.*,c.uom,c.qty,CAST(b.CostOfSales as float) cos from TempRecount a 
				left join Products b on a.ProductID=b.ProductID 
				left join POS_Products c on a.Barcode = c.Barcode
				ORDER BY b.Description ASC";
	...
	//		if($connected){
		//		$this->bdb->trans_start();		
				...
				where a.Branch = '$user_branch' AND a.DateTrans > '2020-01-01 00:00:00.000'
...
function get_cost($stock_id){
	...
	//	$connected = $this->bdb->initialize();
		//	if($connected){
			//	$this->bdb->trans_start();
				$sql = "SELECT * FROM POS_Products where Barcode ='".$bcode."' ";
				$query = $this->bdb->query($sql);
				$result = $query->result();
		//		$this->bdb->trans_complete();
				return $result;
