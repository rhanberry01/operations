<script>
	...
	$('.qty').change(function(){
		...
		$('.rcqty').unbind('input').on('input',function(e)
		{
			...
			var Barcode_cc = $('#Barcode_cc_'+ref).val();
			var user_x = $('#user_x').val();
			var userdb = $('#userdb'+ref).val();

			if(!userdb){
				var update_url = '<?php echo base_url(); ?>operation/updateuser';
				$.post(update_url,{user_x:user_x,Barcode_cc:Barcode_cc}, function( result ) {
							console.log('user updated'+user_x+':'+Barcode_cc);
		                });
			}

			var neg_qty =  $('#neg_qty_'+ref).val();
		//	console.log('neg_qty>+'+neg_qty);
			var qty = parseFloat($('#rcqty_'+ref).val());
			var ad_qty = $('#ad_qty2_'+ref).val();
			var qtyuom = $('#qtyuom_'+ref).val();
			var cost = $('#cost_'+ref).val();
			var csold = 0 ;
			if( parseFloat($('#csold_'+ref).val()) ){
				csold =  parseFloat($('#csold_'+ref).val());
			}
			var prod_code = $('#prod_code_'+ref).val();
			var desc = $('#prod_desc_'+ref).val();

			var holder1 = parseFloat(qty)*parseFloat(qtyuom);
			console.log(holder1);
			var bagoadjqty = holder1  - parseFloat(neg_qty);
			...
			var prod_url =  '<?php echo base_url(); ?>operation/get_prod_uom2/'+Barcode_cc;
			var k = 0;
			var total = 0;
			...
			if (cost == '') {
				...
				cost = parseFloat($('#cost_'+ref).val()) * parseFloat(qtyuom);
			}
			/*
			console.log('cost'+cost);
			console.log('csold'+csold);
			console.log('bagoadjqty'+bagoadjqty);
			*/

			if(qty !==''){
				var total_ad =parseFloat(-qty) + parseFloat(neg_qty) ;
				var subtotal =parseFloat(bagoadjqty) * parseFloat(cost);
				$('#ad_qty2_'+ref).val(Math.abs(bagoadjqty) - csold);
				$('#unit_cost2_'+ref).val(Math.abs(cost));
			...
			var tot = $('#ad_qty2_'+ref).val() * ( parseFloat($('#unit_cost2_'+ref).val()) / parseFloat(qtyuom) );
 ...
 //       					//	$(".right-side").toggleClass("strech");
    ...
    //    				 			$(".right-side").toggleClass("strech");
		...
		$("#auto_form").unbind('submit').submit(function(e) {
			...
			var date_adjustment = $('#date_adjustment').val();
			var infoid = $('#infoid').val();
			$(".actual-count").each(function(){
				if($(this).val() == "")
				validActualCount = false;
			});
			$(".ccqty").each(function(){
				...
				} else if(!validActualCount){
					showAlert('Oops! Actual Count is Required','danger');
				}
				else
				{
				var disabled = $('#chk_discursold').prop('checked');
				var passAdjust = true;
				if(disabled){
					passAdjust = confirm("Currnetly Sold is disabled. Do you wish to continue?");
				}
				if(passAdjust ){
				var post_url = baseUrl+'operation/add_adjustment_headercc/'+infoid;
				var img = "<img src='<?php echo base_url(); ?>/img/ajax-loader2.gif' style='height:100px;'>";
				if(conso_igsa_data.length > 0 || conso_ignsa_data.length > 0){
					var allow = confirm("Are you sure you want to Save Adjustments?");
					if(allow){
		...
		$("#auto_form_").unbind('submit').submit(function(e) {
			...
			$("#pass").keyup(function(event){
			    if(event.keyCode == 13){
			        $("#confirm-certify").click();
						...
						//disable/enable current sales - Van
			$('#chk_discursold').unbind('input').on('click',function(e){
				var check = $('#chk_discursold').prop('checked');
				if(check){
					check = confirm("Are you sure you want to disable Currently Sold?");
					$(this).prop('checked',check);
				}else{
					check = confirm("Are you sure you want to enable< Currently Sold?");
					$(this).prop('checked',!check);
				}
				calculateVariance();
			});
			...
			$('.actual-count').unbind('input').on('input',function(e){
				...
				var currently_sold = $.trim($(this).parent().next().html());
				var variance = actual_count - (system_inventory - currently_sold);
				var check = $('#chk_discursold').prop('checked');
				if(check)
					variance = actual_count - system_inventory;
				$(this).parent().nextAll().eq(1).find('.ccqty').val(variance);
				calculateTotalCostAdjustment($(this).parent().nextAll().eq(1).find('.ccqty'));
				
				if(actual_count == "")
				{
					$(this).parent().nextAll().eq(1).find('.ccqty').val("");
					$(this).parent().nextAll().eq(2).find('.allunitcost').val("");
				}
			});
			
			function calculateVariance(){
				$.each($('.actual-count'),function(){
					var actual_count = $(this).val();
				var system_inventory = $(this).prev().val();
				var currently_sold = $.trim($(this).parent().next().html());
				var variance = actual_count - (system_inventory - currently_sold);
